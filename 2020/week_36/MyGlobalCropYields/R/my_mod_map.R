...
my_mod_map_ui <- function(id){
  ...
  fullPage::pageContainer(
    pageContainer(
      ...
      shinycssloaders::withSpinner(type = 5, color = 'black', echarts4r::echarts4rOutput(ns("map"), height = "50vh")),
      br(), br(),
...
#'
#' @noRd 
my_mod_map_server <- function(input, output, session) {
  ns <- session$ns 
