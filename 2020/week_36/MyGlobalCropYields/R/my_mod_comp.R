...
my_mod_comp_ui <- function(id){
  ...
  fullPage::pageContainer(
    pageContainer(
      ...
      shinycssloaders::withSpinner(type = 5, color = 'black', echarts4r::echarts4rOutput(ns("comp"), height = "50vh")),
      br(), br(), br(),
      p("Inspiration: ",
...
#'
#' @noRd 
my_mod_comp_server <- function(input, output, session) {
  ns <- session$ns 
